...
export function PollingSection({ title, options, pollType }: PollingSectionProps) {
  ...
  useEffect(() => {
    console.log(`PollingSection ${pollType} mounted`);
  ...
  const fetchVotes = async () => {
    try {
      ...
      console.log(`Votes fetched for ${pollType}:`, data);
      ...
      setLoading(false);
      setError(`Koneksi ke server gagal. Pastikan tabel 'polls' sudah dibuat di Supabase.`);
    }
  };
  ...
  const fetchUserChoice = async () => {
    try {
      ...
      console.log(`Fetching user choice from: ${url}`);
      ...
      if (!response.ok) {
        console.error('Failed to fetch user choice, status:', response.status);
        return;
      }
      
      const data = await response.json();
      console.log(`User choice for ${pollType}:`, data);
  ...
  const handleOptionClick = async (option: string) => {
    console.log(`Option clicked: ${option}, current saved: ${savedOption}`);
    ...
    if (savedOption === option) {
      console.log(`Cancelling vote for: ${option}`);
      await deleteVote();
  ...
  const saveVote = async (option: string) => {
    setSaving(true);
    console.log(`Saving vote: ${option} for ${pollType}`);
    ...
    try {
      ...
      setError("Gagal menyimpan vote. Cek console untuk detail.");
  ...
  const deleteVote = async () => {
    try {
      ...
      console.log(`Deleting vote from: ${url}`);
      ...
      const data = await response.json();
      console.log(`Vote deleted successfully:`, data);
